...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            assert new_contain_old or old_contain_new ,'Two ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while checking_place is not beehive:
            if checking_place is not None and len(checking_place.bees) > 0 and self.min_range <= move <= self.max_range:
                return rANTdom_else_none(checking_place.bees)
            elif checking_place is not None and checking_place.entrance is not None:
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.true_queue == False:
            QueenAnt.true_queue = True
    ...
    def action(self, gamestate):
        ...
        if self.is_true_queen:
            ...
            while now_at:
                if now_at.ant != None and now_at.ant.is_doubled == False:
                    now_at.ant.damage = 2 * now_at.ant.damage
                    now_at.ant.is_doubled = True
                now_at = now_at.exit
        else:
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and self.is_true_queen == True:
            bees_win()
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        elif self.armor > 0 and destination is not None and destination != gamestate.beehive:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while now_at != beehive:
            if now_at.ant and self != now_at.ant:
                insects[now_at.ant] = distance
                
            for bee in now_at.bees:
                insects[bee] = distance
            distance += 1
            now_at = now_at.entrance
        return insects     
        # END Problem Optional 5
