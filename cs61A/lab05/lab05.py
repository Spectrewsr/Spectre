...
def add_trees(t1, t2):
    ...
    index = 0
    merge_tree = []
    branch_one , branch_two = branches(t1), branches(t2)
    while index < min ( len(branch_one), len(branch_two)):
        merge_tree += [add_trees( branch_one[index], branch_two[index] )]
        index += 1
        
    merge_tree += branch_one[index:]
    merge_tree += branch_two[index:]
