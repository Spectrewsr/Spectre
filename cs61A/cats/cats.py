...
def about(topic):
    ...
    # END PROBLEM 2
    def check(paragraph):
        lower_paragraph = remove_punctuation(paragraph).lower()
...
def shifty_shifts(start, goal, limit):
    ...
    if start == '' or goal == '':
        ...
        return shifty_shifts(start[1:], goal[1:], limit - int(start[0] != goal[0]) ) + int(start[0] != goal[0])
...
def time_per_word(times_per_player, words):
    """给定时间数据，返回一个包含单词列表和每个玩家打字每个单词所花费时间的游戏数据抽象。

    参数：
        times_per_player: 包含每个玩家时间戳的列表的列表，包括玩家开始打字的时间，然后是每个单词打完的时间。
        words: 单词列表，按照打字顺序排列。
    """
    # 开始问题 9
    ...
    for player_id in range(player_num):
        count = len(times_per_player[player_id])
        pp = []
        for index in range(1, count):  # 从索引 1 开始计算时间差异
            pp.append(times_per_player[player_id][index] - times_per_player[player_id][index-1])
        
        player_time.append(pp)
