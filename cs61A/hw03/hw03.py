...
def balanced(m):
    ...
    if is_planet(m):
        ...
        if not balanced(left_end) or not balanced(right_end):
            return False
        
        else:
            left_weight, right_weight = total_weight(left_end), total_weight(right_end)
            left_length, right_length = length(left_arm), length(right_arm)
            left_val, right_val = left_weight*left_length, right_weight*right_length
...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        left_branch, right_branch = end(left(m)), end(right(m))
        branches = [totals_tree(left_branch)] + [totals_tree(right_branch)]
...
def preorder(t):
    ...
    if is_leaf(t):
        ...
        bs = [preorder(branch) for branch in branches(t)]
...
def div_interval(x, y):
    ...
    #print(type(reciprocal_y))
    return mul_interval(x, reciprocal_y)


def par1(r1, r2):
    print("p1", mul_interval(r1, r2))
    print("p1", add_interval(r1, r2))
...
def quadratic(x, a, b, c):
    ...
    f_l, f_r = a * l ** 2 + b * l + c, a * r ** 2 + b * r + c
    x0 = -b / (2 * a)
    f_x0 = a * x0 ** 2 + b * x0 + c
    if x0 >= l and x0 <= r:
